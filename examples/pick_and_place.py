...
"""
Reach a top-down pregrasp pose 5 cm above the cube with the Panda arm.

The red mocap marker is automatically kept at the reaching goal so the
controller solves a single, fixed pose-reaching objective instead of a
phase-based pick-and-place state machine.
"""
...
initial_knots = jnp.tile(task.goal_ctrl, (ctrl.num_knots, 1))

goal_marker = np.asarray(task.goal_position_from_data(mj_data))
mj_data.mocap_pos[0] = goal_marker
...
def warmup_controller(
    controller: MPPI, warmup_data: mjx.Data, warmup_knots: jax.Array
) -> tuple[Callable[..., Any], Callable[..., Any], Any]:
    """JIT-compile the planner once for the single reaching objective."""
...
print("JIT-compiling the reaching controller ...")
all_st = time.time()
jit_opt, jit_interp, policy_params = warmup_controller(
    ctrl, mjx_data, initial_knots
)
...
with mujoco.viewer.launch_passive(mj_model, mj_data) as viewer:
    ...
    while viewer.is_running():
        ...
        success = task.goal_reached(mj_data)
